...
defmodule TestcontainersElixir.LogChecker do
  ...
  ## Parameters
    ...
    - `conn`: A connection structure to interact with the Docker Engine API.
  ...
  """
  def wait_for_log(conn, container_id, log_regex, timeout \\ 5000)
  ...
  defp wait_for_log(conn, container_id, log_regex, timeout, start_time)
    ...
    if timeout + start_time < :os.system_time(:millisecond) do
      ...
      if log_comparison(conn, container_id, log_regex) do
        {:ok, :log_is_ready}
      else
        # Sleep for 500 ms, then retry
        :timer.sleep(500)
        wait_for_log(conn, container_id, log_regex, timeout, start_time)
      end
    end
  end

  defp log_comparison(conn, container_id, log_regex) do
    with {:ok, %{body: stdout_log}} when is_binary(stdout_log) <-
           Container.container_logs(conn, container_id, stdout: true) do
